...
"What we are not going to do" prevents scope creep, aligns expectations, and accelerates decisions. A feature without non-goals tends to grow indefinitely.
...
- Documented edge cases (LLMs don't guess extreme cases)
...
### "Spec as a technical task list"
"Create users table, add POST /auth endpoint, integrate with OAuth..." — this is an implementation plan, not a spec. The spec speaks in terms of behavior.

### "Verbal / Slack spec"
Decisions made in conversation without records get lost and generate conflicts. Every spec must exist as a written document.

### "Spec that never changes"
Frozen specs that don't reflect the reality of what was implemented. The spec should be updated when implementation intentionally diverges.
...
| **Open Question** | Unresolved doubt that may impact the design |
