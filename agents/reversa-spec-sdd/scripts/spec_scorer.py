...
def has_numeric_metric(text: str) -> bool:
    # Search for patterns like "< 200ms", ">= 25%", "99.9%", "< 2min"
...
def score_completude(text: str) -> DimensionScore:
    ...
    if rf_count >= 5:
        ...
        score += 2
        dim.issues.append(f"❌ Too few RF requirements ({rf_count}) — incomplete spec")
    else:
    ...
    if placeholders == 0:
        ...
        dim.issues.append(f"❌ {placeholders} unfilled placeholder(s) — incomplete spec")
...
def score_testabilidade(text: str) -> DimensionScore:
    ...
    if has_numeric_metric(goals_content) or has_numeric_metric(text[:2000]):
        ...
        dim.issues.append("⚠️ Success metrics without numeric values — hard to validate")
...
def score_edge_cases(text: str) -> DimensionScore:
    ...
    if ec_count == 0:
        dim.issues.append("❌ CRITICAL: No edge cases defined — implementation won't know how to handle errors")
        dim.raw_score = 0
        return dim
