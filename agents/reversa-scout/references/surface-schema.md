...
File generated by the Scout. Used by other agents as a structured context source.
...
| `rationale` | string | yes | Short phrase explaining why this granularity was chosen. Appears in the menu as the Scout's "Reason:". |
...
| Gherkin specs / behavior-driven E2E (`features/*.feature`, `*.spec.*` BDD) | `use-case` |
| Multiple signals coexisting with similar weight | `hybrid` |
| No clear organization signal | `feature` (fallback, fill `features` with whatever could be extracted) |

### Immutability

After the first run, the orchestrator persists the suggested `granularity` in `.reversa/config.toml` under the `scout_suggestion` field. On re-runs, the Scout may regenerate `surface.json` (the legacy may have changed), but the orchestrator does NOT update `scout_suggestion` in `config.toml` (RF-14 from the specs organization spec).
