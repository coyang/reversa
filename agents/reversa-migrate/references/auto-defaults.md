...
When the user invokes `/reversa-migrate --auto`, the orchestrator skips human pauses and applies these defaults. Before starting, the user notice lists each one. Each auto-applied item is recorded in `ambiguity_log.md` with the tag `auto-decided` for later review.
...
- GAP items and AMBIGUOUS items -> DISCARD with explicit note "auto-discarded, requires review".
...
## Designer
- **Topology (Phase 1)**: accepts the proposed modern topology (option 2). Justification recorded in `topology_decision.md` is the Designer's own; in `ambiguity_log.md` the tag `auto-decided` is added for later review. Rationale: `--auto` is for users who want the recommended path; refusing-to-decide would halt the pipeline and violate the `--auto` contract.
...
- **Missing Discovery prerequisites** (`_reversa_sdd/design-system/` or `_reversa_sdd/ui/inventory.md`): creates a minimal `tokens-derived.md` and builds inventory solely from source code; alerts in `ambiguity_log.md`.
...
> "Warning: `--auto` mode activated. The defaults below will be applied without pause for confirmation:
> - Paradigm Advisor: adopt natural paradigm of the target stack (transformational).
> - Curator: AMBIGUOUS/GAP items will be DISCARDED with note; INFERRED items will be MIGRATED with note.
