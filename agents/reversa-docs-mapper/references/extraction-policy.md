...
Defines when to invoke extraction scripts vs. reuse cache in `.reversa/documentation/assets/data/`.
...
2. The JSON's `mtime` is greater than the maximum `mtime` among all relevant source files:
   - For `modules.json`: maximum `mtime` within the source code (excluding `.reversa/`, `_reversa_sdd/`, `node_modules/`, `.git/`).
   - For `deps.json`: maximum `mtime` of the source code AND of `modules.json`.
...
If the `Analyst` runs before the Mapper or in isolated mode and does not find `modules.json`/`deps.json`, it must invoke the **same scripts** following this same policy. The result is shared: subsequent Mapper will use the cache.
