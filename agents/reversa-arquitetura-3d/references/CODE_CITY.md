...
Established software visualization pattern in 3D: each project file is a **building**, grouped into **districts** that correspond to folders. It allows capturing the size, complexity, and distribution of code in a single glance.
...
- Identifying very large files (tall buildings) or complex files (wide buildings).
- Detecting grouping by folder (cohesive vs. scattered districts).
...
### 2. Calculate district size

The district area is proportional to the number of files. Use simple packing (line by line) or squarified treemap.
...
Districts form the city. For up to ~20 folders, pack in a simple grid. For more, use a treemap.
...
Add a large plane as ground and colored squares demarcating each district.
...
- **Click on building**: focus camera on building (animates `controls.target` to building position).
...
- Above this, group files by folder (one building = one folder with aggregated height of LOC, area by number of files).
- Disable shadows if framerate falls below 30fps (detect via `requestAnimationFrame` timer).

## Optional variants

- **Temporal Code City**: animate growth along project history (each commit makes buildings grow).
- **Code City colored by author**: colors indicate the main maintainer of each file.
