...
Oriented graph of dependencies between modules visualized in 3D, with force simulation (attraction between connected nodes, repulsion between unconnected ones) that distributes the graph in space organically.
...
- Detecting **high coupling** (highly connected nodes stay in the center of the cluster).
- Identifying **central modules** (high fan-in) and **isolated modules**.
- Visualizing **dependency cycles** (visible loops in the simulation).
- Comparing cohesion between folders (modules from the same folder should be close).
...
D3-force style simulation adapted for 3 dimensions. Runs in a loop until it stabilizes, then freezes.
...
Use `InstancedMesh` of spheres for up to 1,000 nodes; above this, billboard with sprites.
...
Curved 3D lines using `BufferGeometry` with `LineSegments` or `TubeGeometry` for edges with volume.
...
For oriented edges with visible arrows, use `ArrowHelper` or small cones near the target.
...
After stabilization, maintain rendering but pause simulation to save CPU.
...
Slider changes reactivate simulation for another 100 frames before freezing again.

## Interaction

- **Hover on node**: tooltip with name, number of dependents (fan-in), dependencies (fan-out).
- **Click on node**: highlights the node and its connected edges, blurs others (reduced opacity).
...
Edges: up to **10,000** with LineSegments. Above this, simplify (show only top N by weight) or use color gradient instead of duplicating geometry.
