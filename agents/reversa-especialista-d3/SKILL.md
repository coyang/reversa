...
2. For hierarchical charts, consult `references/layouts-complexos.md` as required.
...
5. **No `fetch()` for local files**: data comes from `window.RV_DATA.<key>` (loaded by `assets/js/data.js` generated by the Publisher). In standalone mode outside the Docs team, embed data via `<script id="data" type="application/json">{...}</script>`.
