...
Second agent in the `/reversa-docs` pipeline. Reuses the Mapper's intermediate JSONs (`modules.json`, `deps.json`). In isolated invocation, detects absence and runs minimal extraction using the same Mapper scripts.

## Inputs

- `.reversa/documentation/assets/data/modules.json` (from Mapper, or extracts on demand)
...
- `.reversa/documentation/metricas.html` (4+ chart dashboard)
- `.reversa/documentation/timeline.html` (omitted if chronicle absent)
...
4. Check if `.reversa/documentation/assets/vendor/highcharts.js` (and associated modules) exists. If absent in isolated mode, execute Step 0 of the Publisher (`agents/reversa-docs-publisher/SKILL.md`) reading `vendor-pins.yaml` to download Highcharts + modules with CDN retry. In orchestrated mode, this was already done in Phase 0.
...
4. Responsive grid layout 2x2. Add 5th/6th charts if rich data is available (e.g., `language_distribution`).
...
Only writes to `.reversa/documentation/`. `chronicle.md`, `modules.json`, `deps.json` are read without modification.

## Graceful handling

| Absent source | Behavior |
|---|---|
| `modules.json`/`deps.json` (Mapper didn't run) | Invokes extraction scripts before proceeding. |
