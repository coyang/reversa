#!/usr/bin/env python3
"""
Parses data from CSV, JSON or Excel and formats it for use with Highcharts.
...
def detect_encoding(filepath: str) -> str:
    """Attempts to detect the file's encoding."""
...
def parse_csv(filepath: str, encoding: str = 'utf-8', delimiter: str = None) -> dict:
    ...
    # First column = categories, the rest = series
...
def parse_json_data(filepath: str) -> dict:
    ...
    # If already in Highcharts format, return as-is
...
def parse_excel(filepath: str, sheet: str = None) -> dict:
    ...
    if len(rows) < 2:
        return {"error": "Sheet with fewer than 2 rows"}
