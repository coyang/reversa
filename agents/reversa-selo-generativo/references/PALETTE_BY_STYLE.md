...
- `fg`: text label color outside the canvas.
...
**Dark variant**: for use in mini-seal on a dark header, mirror (bg <-> fg).
...
- Accent: light gold (brighter than base gold).
...
Dense, saturated, high visual density style. Focus on distinguishing between multiple categories.
...
When the user chooses "Other" in the style menu and provides a free-form description, the skill maps the description to the nearest palette, or applies a basic heuristic:

```javascript
function paletteFromFreeform(text) {
    const lower = text.toLowerCase();
    if (/(luxury|premium|cinematograph|dark)/.test(lower)) return palettes.premium;
    if (/(technical|sober|clean|minimal)/.test(lower)) return palettes.sober;
    if (/(dense|saturated|colorful|vibra)/.test(lower)) return palettes.dense;
...
Even with 5 colors in `foreground`, the seal does not use them equally. Visual proportion rule:
