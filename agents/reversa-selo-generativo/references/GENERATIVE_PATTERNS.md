...
Catalog of the 5 established patterns that the `selo-generativo` skill produces. Each pattern has a distinct appearance, central algorithm, and parameters derived from the seed.

General seed pattern: the sha256 hash (64 hex chars) is sliced into 8-char chunks, each chunk becomes a `parseInt(slice, 16)` and feeds a distinct parameter. Thus, different patterns from the same seed share a visual personality.

---

## 1. flow-field

Perlin flow fields: thousands of particles follow vectors derived from noise, leaving organic curved trails. "Turbulent natural" style.

**When it fits**: `sober` style (soft version) and `exploratory` style (luminous version).
...
- Field gravity center (if attractor exists): XY coordinate (slices 2 and 3).

**Performance**: up to 1500 particles on an 800x800 canvas without lag.
...
**When it fits**: `premium` style (dark, golden) and `exploratory` style (luminous pastels).
...
**Performance**: trivial, tens of elements.
...
**When it fits**: `dense` style (saturated) and `sober` style (clean).
...
Moiré-style interference patterns: circular waves emanating from multiple centers that intersect, generating complex textures from simple rules.

**When it fits**: `premium` style (black + gold, high contrast) and `dense` style.
...
**When it fits**: `sober` style (neutral terracotta) and `exploratory` style (auroral).
...
function pickPattern(seedHex, styleHint) {
    ...
    // Soft adjustment by style (chooses among "compatible" patterns if there is a mismatch)
...
The `pattern x style` compatibility appears at the beginning of this reference. When there is a declared incompatibility, the `pickCompatible` function re-evaluates among the patterns marked as appropriate for the style.

---

## Manual override

The skill accepts a `forcePattern` parameter to ignore seed derivation and manually choose the pattern, useful when the user wants a specific seal in a style different from the default.
