...
> This copy is installed together with the agent so it has access to the catalog within the user's project, without depending on the npm package location.
...
- **Legacy examples**: modern queue-oriented Node backends, heavy SQS / Kafka systems, async microservices.
...
- **Signals in `_reversa_sdd/`**: DAG description, stage transformations.
...
|---|---|---|
| procedural → event-driven | sync → async | response is no longer immediate; error handling becomes retry/DLQ; idempotency mandatory; event order starts to matter |
| procedural → OO with DI | data as dict → aggregates | invariants move inside aggregates; logic leaves controllers; dependencies via interfaces |
...
| classic OO → event-driven | synchronous flow → choreography | actions cease to be atomic; distributed transactions become sagas; strong consistency → eventual |
...
| classic OO → functional | mutable encapsulation → immutability | effectful methods become pure functions + explicit update; state expressed as sequence of transformations |
...
| OO with DI → functional | mocks → testable composition | DI ceases to be by interface, becomes by function argument |
| functional → event-driven | synchronous composition → messaging | latency increases; failure becomes message in DLQ; distributed state |
