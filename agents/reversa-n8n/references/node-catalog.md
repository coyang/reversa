...
Quick reference used by `reversa-n8n` to interpret node types and suggest equivalent Python libraries. Not exhaustive: covers the most common nodes. When the node type is not listed, deduce it by name (`type` follows the pattern `n8n-nodes-base.<service>` or `@n8n/n8n-nodes-langchain.<service>`) and by `parameters`.
...
| `n8n-nodes-base.intervalTrigger` | Fixed time interval loop | `while True: ... time.sleep(N)` or APScheduler |
...
| `n8n-nodes-base.errorTrigger` | Trigger on error from another workflow | Exception hook, error decorator |

Service triggers (pattern `<service>Trigger`): map to webhooks or polling in the service's SDK (e.g.: `slackTrigger` becomes Slack webhook or polling via `slack-sdk`).
...
| `n8n-nodes-base.merge` | Merge parallel branches | List combination, `dict.update`, `pandas.concat` |
...
| `n8n-nodes-base.noOp` | Direct passthrough | `pass` or identity function |
| `n8n-nodes-base.stopAndError` | Stop with error | `raise Exception(...)` |
| `n8n-nodes-base.executeWorkflow` | Call another workflow | Python function/module call |
...
| Type in JSON | Meaning | Python |
|---|---|---|
| `n8n-nodes-base.set` | Set field values | Dict assignment |
...
When translating Function/Code nodes to `design.md`: describe the logic in pseudocode, do not copy the literal JS. The equivalent Python should be idiomatic.
...
| `n8n-nodes-base.whatsApp` | WhatsApp Business API | `httpx` direct (no mature official Python SDK) |
...
For new projects, consider whether LangChain/LangGraph is necessary or if a direct call to the LLM SDK would be simpler.
...
|---|---|---|
| `httpHeaderAuth` | Custom header (usually API key) | Environment variable, header on all calls |
| `httpBasicAuth` | Basic Auth | `(user, pass)` tuple in `httpx`, from env vars |
| `httpQueryAuth` | API key in querystring | Fixed param on all calls |
...
| Multi-trigger | FastAPI with endpoints + built-in APScheduler |
...
Common behaviors in N8N and Python equivalents:
