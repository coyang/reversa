...
In standalone mode (without `/reversa-migrate` having been run), the Designer prerequisites are dropped; the agent instead asks the user directly for the target platform. Before writing any artifact, ensure that `_reversa_sdd/migration/` and `_reversa_sdd/screens/` exist; create them if necessary (without touching any other path in the project).
...
- `_reversa_sdd/screens/golden/<screen>.<ext>` (optional, when oracle executes)
...
1. **Mandatory human decision on mode.** The agent always presents literal, modernized, and hybrid with concrete trade-offs, recommends one, and never decides alone. It mirrors the pattern of `paradigm_decision.md` and `topology_decision.md`.
...
5. **Read-only on legacy.** The agent never modifies files outside `_reversa_sdd/migration/` and `_reversa_sdd/screens/`.
6. **Does not invent modern states.** In literal mode, the agent only preserves states that the legacy has. In modernized mode, it explicitly declares the 4 states (idle, loading, error, success) per screen.
...
- If it exists but `screenModeApproved` is `false` or absent: the orchestrator erred in re-activating. Exit with a message to the orchestrator requesting human approval before proceeding.
...
See `references/platform-detection.md` for the complete list. Use the scale CONFIRMED / INFERRED / GAP / AMBIGUOUS.

If unable to classify (proprietary framework without known signature): register `EC-01`, signal the user, and offer a raw template.
...
If there is conflict or ambiguity (architecture silent on UI), ask the user with `AskUserQuestion` or equivalent.

In standalone mode (without `/reversa-migrate` having been run), ask for the target platform explicitly. Do not try to guess.
...
- `screen_modernization_decision.md` with `mode: skipped` in the front-matter, reason filled in (e.g.: "Legacy is pure batch, no UI. Internal inventory detected 0 screens; `_reversa_sdd/ui/inventory.md` absent or empty."), and "Evaluated modes" / "Decision" sections marked as N/A.
...
From the detected source->target pair, consult `references/adapter-pairs.md` and select viable modes. For each presented mode, list at least 4 concrete trade-offs with clear grading:
...
Always present up to three options, with label, description, and trade-off grading. Always include a final open "Other" option for unforeseen cases (e.g.: the user wants a custom mode, or to skip translating an entire class of screens).

Ask explicitly: **"Which mode do you choose?"**. In hybrid mode, then ask for the explicit list of which screens go in literal and which in modernized. Refuse if either list is empty (EC-12).
...
Phase 2 only runs after the orchestrator returns approval. Do not write `target_screens.md`, golden files, or deviation log before that.
...
- Screen identity.
- Source in the legacy (`<file:line>`).
- Applied mode.
...
Capture must be deterministic: fake clock, fixed seed, no dependency on external clock. If determinism fails for a screen, document it in `screen_deviation_log.md` and offer capture by sampling (RF-21).

In v1, **do not** attempt to automate drivers for Docker/Wine/emulator. Emit the `manifest.yaml` (template in `references/templates/golden_manifest.yaml`) listing the suggested capture command per screen, and instruct the user to run it manually when the oracle allows. Automated capture is OQ-02 and is left for v2.
...
| EC-18 | `_reversa_sdd/ui/inventory.md` absent | Alerts user, offers to run `reversa-visor` first; in `--auto` mode builds inventory only from source code |
...
- Never write outside `_reversa_sdd/migration/`, `_reversa_sdd/screens/`, and `_reversa_sdd/design-system/tokens-derived.md`.
- Phase 2 can only run after the user approves `screen_modernization_decision.md`. Never apply modernization silently.
- Literal textual content by default. Linguistic revision only with explicit approval recorded in the decision.
...
- Unsupported source->target pairs in v1 return `EC-01` and offer a raw template; never improvises a format.
