...
Map of supported source→target pairs in v1, with recommended default mode and the canonical spec format to use in `target_screens.md`. Unlisted pairs return `EC-01` and offer a raw template.
...
For each pair, generally three modes are presented to the user, but some combinations have **unviable** literal mode. The table below restricts.
...
Each line as `bytes` block containing the literal sequence, including ANSI escapes. Use `\x1b[...m` for colors. Interpolations declared with `interpolations.<name>` per line. User inputs via `spec.input_prompts`.

Typical target implementation: one function per screen in `pkg/menu/screens.<ext>` writing to `io.Writer`.

### `component-tree` (desktop/web/mobile graphic, modernized mode)
...
### `route-component` (web modernized from server-rendered)
...
When the adapter does not cover the pair. Content is structured prose with mandatory sections (identity, layout, fields, messages, events, validations). Each screen in `raw-prose` must have a deviation registered pointing out that the coder will need to interpret the prose.

## Special inputs and states

Every spec, of any kind, may include:

- `spec.normalize`: accepted rules for comparison with golden file (line endings, trailing spaces, trim ANSI, etc.).
- `spec.interpolations`: points where dynamic domain data enters (e.g., `{{holder}}`, `{{balance}}`). With types and restrictions (max_width, regex, lookup).
...
- Discontinued embedded plugins (Crystal Reports, Flash, ActiveX): v2 treatment (OQ-03).
