...
> Conscious decision on how to organize the new system: preserve the legacy topology, adopt a modern topology, or apply a hybrid approach.
> This artifact is mandatory reading for the Designer (to decompose bounded contexts) and for the coding agent (to create the folder tree).
...
- **Confidence**: GREEN CONFIRMED | YELLOW INFERRED | RED GAP | WARNING AMBIGUOUS
- **Evidence**:
  - <evidence 1, with reference to artifact in `_reversa_sdd/` (architecture.md, inventory.md, dependencies.md)>
  - <evidence 2>
...
1. **Preserve legacy topology** (conservative)
   - Consequences: maintains current team's mental map; perpetuates any structural debt; reduces migration risk.
2. **Adopt proposed modern topology** (transformational)
   - Consequences: breaks structural debt; requires learning; maximizes gains from the target stack.
3. **Hybrid** (balanced)
...
## Pending implications for Designer's next steps
...
## Notes
<Any additional points the coding agent needs to know to create the folder tree and respect the chosen topology.>
