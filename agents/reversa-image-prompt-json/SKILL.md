...
description: >
  Creates structured JSON prompts for high-quality image generation with luxurious
  and cinematic aesthetics. Use this skill whenever the user wants to generate an image
  prompt, create a product photo, build a prompt for an image AI, virtually photograph
  a product, create images of food, beverages, cosmetics, jewelry, fashion, or any
  visual item. It should also be activated when the user mentions: "image prompt",
...
7. **Suspended elements in the air**: Which elements fly around the product? (e.g., drops, powder, fragments, leaves, crystals, bubbles...)
...
14. **Aspect Ratio**: What is the image proportion? (default: `16:9`)
...
With confirmed answers, build the prompt following **exactly** this schema:
...
```json
{
  "master_prompt": {
    ...
    "product": {
      "type": "[rich and adjective-heavy description of the product]",
    ...
    "composition": {
      ...
      "placement": "[central hero positioning on the specified surface]"
    },
    "lighting": {
...
- The product is always the **central hero** of the scene
...
Present the JSON formatted in a code block and add:
