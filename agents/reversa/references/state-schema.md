...
The specs organization decision (granularity, custom folders, Scout's original suggestion, choice timestamp) does **not** go in `state.json`. It is persisted in `.reversa/config.toml`, section `[specs]`, as described in `references/step-03-specs-organization.md`. The `state.json` is runtime state; the `config.toml` is a long-term decision.
