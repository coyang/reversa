...
> This step only runs on **re-extractions**, that is, when a reverse pipeline is executed on a project that has already gone through at least one `/reversa-coding` cycle. On projects without `_reversa_forward/` or without `regression-watch.md`, this step is silently skipped.

## Why it exists

Reversa is not just one-shot extraction. Each `/reversa-coding` leaves in `_reversa_forward/<feature>/regression-watch.md` a list of rules that must remain true in the next extraction. The reverse pipeline, when re-running, has the duty to check these rules against the current code and report regressions. This is Reversa's competitive differentiator over pure forward frameworks.

## When to run

After the **last agent in the plan** completes, before the final "extraction completed" message. The trigger is position (last item in `.reversa/plan.md`), not agent name, because the last agent varies depending on the optionals selected at install (Reviewer may be absent, for example). Perform the checks in this order:

1. Check if `_reversa_forward/` exists at the project root. If it doesn't, silently end this step.
...
2. For each watch item in the main table (not archived ones):
   ...
   2.2. Apply the corresponding verification against the newly generated artifacts in `_reversa_sdd/`:
        - `presence`: the rule needs to be present in `_reversa_sdd/domain.md` (or in the file pointed to by the Origin column) with the same semantic essence.
        - `absence`: the original rule must NO longer appear in the SDD.
        - `wording`: the text was deliberately changed; verify that the new version matches the expectation.
        - `confidence`: the rule is still present, but the confidence (green, yellow, red) must be equal to or greater than expected.
...
| W010 | yellow | equivalent text but literally different; awaits judgment |
```
...
If `setup.json#watch.block-on-red` is `true`, suggest the user **not** proceed with new `/reversa-requirements` until each red has been triaged. Reversa only alerts; it never automatically blocks the user's flow.

## Special case, no `_reversa_sdd/`

If during the procedure `_reversa_sdd/` does not have the expected files (because the re-extraction was partial or the documentation level was reduced), record a yellow verdict with the note `evidence missing, _reversa_sdd/<file> was not generated in this extraction` and continue.

## Known limitation

Semantic equivalence between expected rule and extracted rule is a subjective evaluation. When in doubt, prefer a yellow verdict. Red verdicts should be reserved for cases where the rule simply disappeared or was explicitly contradicted.
