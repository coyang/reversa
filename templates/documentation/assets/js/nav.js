 ...
 *
 * Marks the link of the current page as active,
 * allows theme toggling via the header button,
...
(function () {
     ...
     * If the Publisher has already filled it via NAV_LINKS server-side, keep it.
     * If the nav is empty (partial-regeneration case), fill it here.
    ...
    function renderNav() {
        ...
        if (!nav) return;
        // If nav already has <a>, respect the server-side render.
    ...
    /**
     * Marks the nav link that points to the current page.
     */
     ...
     * j/k to move forward/back between content sections,
