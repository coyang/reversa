...
export class Writer {
  ...
  // Installs the skills of an agent for an engine
  ...
  // force=true: silently overwrites (used by update on intact files)
  async installEntryFile(engine, { force = false } = {}) {
    ...
    // File already exists — ask the user (only merge or skip)
  ...
  createReversaDir(answers, version) {
    ...
    state.autopilot = answers.autopilot ?? 'full';
    state.doc_level = answers.doc_level ?? 'complete';
    ...
    // Always overwrite state.json — even on retry/partial install.
    // _writeNew skips existing files, but a corrupt or stale state.json
    // would crash saveCreatedFiles() later.
    this._mkdir(dirname(statePath));
    writeFileSync(statePath, JSON.stringify(state, null, 2), 'utf8');
    this._register(statePath);
    ...
    const config = configTemplate
      ...
      .replace('autopilot = "full"', `autopilot = "${answers.autopilot ?? 'full'}"`)
      .replace('doc_level = "complete"', `doc_level = "${answers.doc_level ?? 'complete'}"`);
  ...
  // skipping files modified by the user. setup.json is always preserved because it carries
