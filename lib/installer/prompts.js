...
export async function runInstallPrompts(detectedEngines) {
  ...
  const answers = await inquirer.prompt([
    ...
    },
    {
      ...
      type: 'list',
      name: 'chat_language',
      message: promptTitle(5, 'Language for agent interactions:', 'list'),
      loop: false,
      choices: [
    ...
    },
    {
      ...
      type: 'list',
      name: 'doc_language',
      message: promptTitle(6, 'Language for generated documents and specs:', 'list'),
      loop: false,
      choices: [
    ...
    },
    {
      ...
      type: 'list',
      name: 'autopilot',
      message: promptTitle(10, 'Auto-continue between generated files / agents?', 'list'),
      loop: false,
      choices: [
        { name: 'Off — pause after every file',                              value: 'off'  },
        { name: 'Unit — pause only between units / modules',                value: 'unit' },
        { name: 'Full — run the whole plan without pausing (recommended)',   value: 'full' },
      ],
      default: 'full',
    },
    {
      ...
      type: 'list',
      name: 'doc_level',
      message: promptTitle(11, 'Documentation level — volume of artifacts each agent will generate?', 'list'),
      loop: false,
      choices: [
        { name: 'Essential — code-analysis, domain, architecture (C4 context), SDD specs. Ideal for simple projects, scripts, prototypes.',  value: 'essential' },
        { name: 'Complete — everything in Essential + full C4 diagrams, ERD, ADRs, OpenAPI, user stories, traceability matrices. (recommended)', value: 'complete' },
        { name: 'Detailed — everything in Complete + per-function flowcharts, expanded ADRs, deployment diagram, mandatory cross-review. For enterprise systems.', value: 'detailed' },
      ],
      default: 'complete',
    },
  ]);
